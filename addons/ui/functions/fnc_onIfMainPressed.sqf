...
if (!isNil QGVAR(ifOpen) && {GVAR(ifOpen) select 0 == _ifTypesPrimary}) exitWith {
	// close Main
	call FUNC(close);
...
private _interfaceName = call {
	...
	//CC: Main order: TAD, Android, DAGR, FBCB2 Vehicle, Tablet
	...
	// main == dsp
	if ([_player,["ItemAndroid"]] call EFUNC(core,checkGear)) exitWith {QGVARMAIN(Android_dsp)};
...
if (_interfaceName != "" && _interfaceName != _previousInterface) exitWith {
	...
	},0,[_ifTypesPrimary,_interfaceName,_player,_vehicle]] call CBA_fnc_addPerFrameHandler;
