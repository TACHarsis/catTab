...
private _fnc_toggleAssignedFrame = {
    ...
    LOG_3("ALT+%1 pressed. Folding %2? [%3]",keyName _dikCode,_videoSlotUI,ctrlshown _videoSlotUI);
    if(ctrlShown _videoSlotUI) then {
        private _fnc_fold = (_videoSlotUI getVariable QGVAR(fnc_fold));
        [_videoSlotUI] call _fnc_fold;
    };
};
switch (true) do {
    case (_ctrlKey) : {
        if (!isNull _selectedSource) then { // we have sth selected already
            if (_selectedSource isNotEqualTo _assignedSource) then { // it's different from the source in the slot
                ...
                LOG_3("CTRL+%1 pressed. Saving selected %2 to frame %3",keyName _dikCode,_selectedSource,_slotIdx);
            ...
            if !(isNull _closestSource) then { // there is a valid source nearby
                ...
                LOG_3("CTRL+%1 pressed. Saving nearby %2 to frame %3",keyName _dikCode,_closestSource,_slotIdx);
                ...
                LOG_2("CTRL+%1 pressed. Nothing selected nor nearby. Deselecting frame %2",keyName _dikCode,_slotIdx);
                [] call _fnc_select;
    ...
    default {
        if !(isNull _assignedSource) then {
            if(_assignedSource isNotEqualTo _selectedSource) then { // we do not have it selected right now
                ...
                LOG_3("Pressed %1, selecting %2 from slot %3.",keyName _dikCode,_assignedSource,_slotIdx);
            ...
            LOG_2("Pressed %1, but there was no source assigned to slot %2.",keyName _dikCode,_slotIdx);
            [] call _fnc_select;
        };
    };
};
